...
class SimpleChainHash {
 ...
 public SimpleChainHash(int num) {
	 ...
	 for(int i=0; i<size; i++)
		 table[i] = new Node(); // null이 아니라 Node객체가 만들어진것 조심하기!
 }
 ...
 public int add(int key) {
	 int hash = hashValue(key);
	 Node p = table(hash);
	 while(p != null) { // 충돌나는 것 제외하면 나머지는 null이 아니게 초기화해줬으므로 충돌나는 것만 탐색
		 if(p.key == key)
			 return 1;
		 p = p.next;
	 }
	 Node tmp = new Node(key);
//	 Node tmp = new Node(key, table[hash]); // 이 구문을 활용한 방법으로도 해보기!
	 tmp.next = table[hash];
	 table[hash] = tmp;
	 return 0;
 }
