...
import 자료구조제10장해시.OpenHash2.Bucket;
...
//--- 생성자(constructor) ---//
  public OpenHash2(int size) {
	  this.size = size;
	  table = new Bucket[size];// 배열
	  for(int i=0; i<size; i++) {
		  table[i] = new Bucket(); // 필요한 이유를 이해
	  }
...
//           */
//          for (int i = 0; i < size; i++)
//              table[i] = new Bucket();//실제로 각 인덱스에 Bucket 객체를 생성해서 넣는 과정
//          this.size = size;
...
public int add(int data) {
    ...
    Bucket p = table[hash];
    for(int i=0; i<size; i++) { // 해시 테이블을 모두 방문하면 종료
