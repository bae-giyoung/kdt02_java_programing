...
//--- 스택에 x를 푸시 : top 증가, 반환값은 push한 값 ---//
	public int push(int x) throws OverflowIntStackException {
		...
		top++;
		return x;
		// return stk[top++] = x; // 한 줄로 코딩
//추가
	}

//--- 스택에서 데이터를 팝(정상에 있는 데이터를 꺼냄) : top 감소, 반환값은 pop한 값 ---//
	public int pop() throws EmptyIntStackException {
		if(isEmpty())
			throw new EmptyIntStackException("pop: the stack is empty");
		return stk[--top];
//추가
	}

//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄): 반환값 꼭대기에 있는 요소 ---//
